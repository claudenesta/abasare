  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-12">
      <div class="box box-success">
        <div class="box-body">  
          <h2>Monthly Report on Savings </h2>
          <div class="row">
            <div class="col-xs-12">
              <div class="table-responsive" data-pattern="priority-columns">
                              <table class="table table-bordered table-hover" id="example1">
                  <caption class="text-center">Saving Status Information by member's <a href="#" target="_blank"> info</a>:</caption>
...
$month = strtotime($yee);
//$yaer=date('Y');
$yaer=@$_GET['year']?$_GET['year']:date('Y',strtotime("-1 year"));
while($i <= 12)
{
    
$sql_q=mysqli_query($con,"SELECT sum(saving.sav_amount) as amount,CONCAT(member.fname,' ',member.lname) as fullname, saving.month FROM saving,member WHERE saving.member_id=member.id and saving.year='".$yaer."' AND member.id='".$iid."' AND saving.month='".$i."' GROUP BY member.id,saving.month");
$row2=mysqli_fetch_array($sql_q);    
 echo '<td data-priority="'.$i.'">'.number_format($row2['amount']).'</td>';
    $i++;
}
?>
</tr><?php } ?>
                ...
                <a href="./export_monthly_savings_status.php" target="_blank" class="btn btn-success">Download</a>
              </div>
            </div>
          </div>
        </div>
...
<script type="text/javascript">
  ...
  $("#year").on('change', function(e){
    ...
    // $("#report_containter").load("./savings_info.php?year=" + $(this).val(), function(){
    //   $("#example1").DataTable();
    // });
  });
