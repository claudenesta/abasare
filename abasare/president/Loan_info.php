...
$loan="SELECT * , 
              ml.id AS borrower_loan_id, mls.lname AS loan_name,
              ml.status
...
$rows= first($db, $loan, [$loan_id]);
// var_dump($rows);

$remainder = returnSingleField($db, "SELECT amount FROM loan_remainder WHERE loan_id = ?", "amount", [$loan_id]);
if(is_null($remainder)){
  $remainder = 0;
}

$paidloan="SELECT (sum(amount) + {$remainder}) as total FROM lend_payments WHERE borrower_loan_id=? AND status='PAID'";
// $qqu=mysqli_query($con,$paidloan);
$result=first($db, $paidloan, [$loan_id]);
    ...
    <section class="content">
      <div class="row">
        ...
        <div class="row">
          <div class="col-md-3">
            ...
            <div class="card card-primary card-outline">
              ...
              <div class="user-block">
                ...
                <p class="text"> Remaining Balance: <?php echo number_format($rows['loan_amount']-(int)$result['total']);  ?> </p>
                <p class="text"> Payments Made: <?php echo number_format((int)$result['total']);?> Frw(s) </p>
                <p class="text"> Total Loan Amount: <?php echo number_format((int)$rows['loan_amount']);?> Frw(s)  </p>
              <hr>
                  <div class="user-block">
                    <span class="username">
          ...
          // var_dump($rows);
          // die($payid);
			   $pay="SELECT * from lend_payments lp inner join member_loan_settings ms on ms.loan_id=lp.borrower_loan_id where lp.payment_number=?";
         // echo $pay;
			   // $query=mysqli_query($con,$pay);
			   $roww=first($db, $pay, [$payid]);
			   ?> </p>
			   
			    
                <p class="text"> Amount : <?php echo number_format($rows['loan_amount_term']);  ?> Frw(s) </p>
	  ...
	  $payid=$row['payment_number'];
		?>
                   <tr>
					  ...
					  <td><?php echo number_format($amont); ?></td>
