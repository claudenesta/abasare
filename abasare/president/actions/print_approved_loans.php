...
$loans = returnAllData($db, $sql = "SELECT a.id,
									...
									ON a.president = e.id
									INNER JOIN member_loan_settings AS f
									ON a.id = f.loan_id
...
if(count($loans) > 0){
	...
	foreach($loans AS $loan){
		$data .= "<tr>";
			$data .= "<td>".($count++)."</td>";
			$data .= "<td>{$loan['president_date']}</td>";
			...
			$data .= "<td>".number_format($loan['loan_amount'] * $loan['interest']/100) ."</td>";
			$data .= "<td>".number_format($loan['installment']) ."</td>";
			$data .= "<td>".number_format($loan['saving']) ."</td>";
