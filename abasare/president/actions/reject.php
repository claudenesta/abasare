...
$db->beginTransaction();
try{
	saveData($db, "UPDATE member_loans SET status=?, president_status=?, president=?, president_date=?, president_reject_comment=? WHERE id = ?", ["REJECTED", 0, $_SESSION['user']['id'], (new \DateTime())->format("Y-m-d"), $_POST['comment'] , $_POST['loan_id']]);
