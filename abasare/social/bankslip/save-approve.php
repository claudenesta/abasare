...
try{
	...
	$saving_id = saveAndReturnID($db, "INSERT INTO sacial_saving SET m_id=?, amount=?, month=?, year=?, confirmed_date=?", [$request['member_id'], $data->sav_amount, $data->month, $data->year, (new \DateTime())->format("Y-m-d H:i:s")]);

	if($request['has_fine'] == 1){
		$fine_info = json_decode($request['fine_data']);
		saveData($db, "INSERT INTO interest SET member_id=?, amount=?, saving_overdu=?, ref_id=?, desciption=?, month=?, year=?, done_at=?", [$fine_info->member_id, $fine_info->amount, $fine_info->saving_overdu, $saving_id, $fine_info->desciption, $fine_info->month, $fine_info->year, $request['created_at']]);
	}
