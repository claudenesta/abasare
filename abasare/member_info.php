...
$membe_id=$_GET['id'];
$accloan = "SELECT CONCAT(s.year, '-', IF(s.month<10,0,''), s.month, '-01') AS month 
                       FROM `sacial_saving` s 
                       where s.m_id='$membe_id' 
...
$saving_sql = "SELECT CONCAT(s.year, '-', IF(s.month<10,0,''), s.month, '-01') AS month 
                       FROM `sacial_saving` s 
                       where s.m_id='$membe_id' 
...
//check if any missing contibution during social savings operation



$startmonth = new \DateTime($row2['month']);
$endMonth = new \DateTime($row0['month']);
$required_month = "";
$found_month = "";
$my_counter = 0;
while($startmonth->getTimestamp() <= $endMonth->getTimestamp()){
  if($my_counter++ > 0){
    $required_month .= " UNION ";
    $found_month .= " UNION ";
  }
  $required_month .= "SELECT '".$startmonth->format('Y')."-".$startmonth->format('n')."' AS month";
  $found_month .= "SELECT CONCAT(year,'-',month) AS month FROM sacial_saving WHERE month='".$startmonth->format('n')."' AND year='".$startmonth->format('Y')."' AND m_id = '{$membe_id}'";
  $startmonth->modify("+1 month");

}
$missing_month_query = "SELECT COUNT(a.month) AS missing_months
              FROM (
                  SELECT  a.*,
                          b.month AS avaialable_month
                          FROM (".$required_month.") AS a
                          LEFT JOIN (".$found_month.") AS b
                          ON a.month = b.month
                          HAVING avaialable_month IS NULL
              ) AS a
              "; 
$querprofile=mysqli_query($con,$missing_month_query);
            ...
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
				...
				<?php if($rows['is_new']==0){?>
				 <!-- <a class="btn btn-success btn-flat btn-sm" data-toggle="modal" data-target="#modal-saving"><i class="fa fa-plus"></i> Upload Photo </a> -->
                          ...
                          <div class="col-sm-3">
                            <div class="card">
                              <div class="card-body">
                                ...
                                <a href="#" class="btn btn-primary"><?php echo number_format(ceil($row1['gtotal1'])); ?> Frws</a>
                              </div>
                            </div>
                          </div>
                          <div class="col-sm-3">
                            <div class="card">
                              <div class="card-body">
                                <h5 class="card-title">First Month</h5>
                                <a href="#" class="btn btn-primary"><?php echo (new \DateTime($row2['month']))->format('F Y') ?> </a>
                              </div>
                            </div>
                          </div>
                          <div class="col-sm-3">
                            <div class="card">
                              <div class="card-body">
                                ...
                                <a href="#" class="btn btn-primary"><?php echo (new \DateTime($row0['month']))->format('F Y') ?> </a>
                              </div>
                            </div>
                          </div>
                          <div class="col-sm-3">
                            <div class="card">
                              <div class="card-body">
                                <h5 class="card-title">Missing Savings</h5>
                                <a href="#" class="btn btn-danger btn-block"><?php echo number_format(ceil($profi['missing_months'])); ?></a>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="box-body">
                              ...
                              <div class="box box-primary">
                                <div class="box-header with-border">
                                  <h3 class="box-title" style="color:#008080"> Social Saving Statement </h3>
                                </div>
                                <!-- /.box-header -->
                                <div class="box-body ">
                                          ...
                                          <tbody>
                                            ...
                                            // echo $sql;
                                              ...
                                              <tr>
                                                ...
                                                if($_SESSION['role']==7){
                                                    ?>
                                                <td> <a href="delete_social.php?id=<?= $row['id'] ?>&m_id=<?= $row['m_idi'] ?>">Delete/Edit</a></td> <?php } ?>
                                              </tr>
                                              <?php
                                            } 
                                            ?>
