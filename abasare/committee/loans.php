...
$loans = returnAllData($db, $sql = "SELECT a.id,
									...
									LEFT JOIN (
										SELECT 	a.member_id,
												...
												WHERE a.member_id IN (
													SELECT member_id FROM member_loans WHERE committee_status IS NULL AND signatory_2_status = ?
												)
										...
										SELECT 	a.member_id,
												...
												LEFT JOIN (
													SELECT 	b.member_id,
															...
															WHERE b.status = ? AND b.member_id IN (SELECT member_id FROM member_loans WHERE committee_status IS NULL AND signatory_2_status = ?) AND a.status = ?
															GROUP BY b.member_id
													) AS b
												ON a.member_id = b.member_id
												WHERE a.status = ? AND a.member_id IN (SELECT member_id FROM member_loans WHERE committee_status IS NULL AND signatory_2_status = ?)
												GROUP BY a.member_id
										) AS e
									ON b.id = e.member_id
									INNER JOIN (
									...
									WHERE a.committee_status IS NULL
									AND a.signatory_2_status = ?
		...
		<tbody>
			...
			foreach($loans AS $loan){
				...
				<tr>
					...
					<td><?= number_format($loan['capital_share']) ?></td>
					<td>
